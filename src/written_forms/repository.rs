...
pub fn save_written_form(
    ...
    let entity_id = preview.expected_entity_id.unwrap();
    ...
    tx.execute(
        "INSERT INTO written_form_store_members (store_entity_id, written_form_entity_id, admitted_revision_cid, added_at, status)
         VALUES (?1, ?2, ?3, datetime('now'), ?4)",
        params![STORE_ENTITY_ID, entity_id, cid_str, "active"],
    )?;
